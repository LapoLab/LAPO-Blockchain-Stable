...
*Setup instructions for a gitian build of LAPO using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LAPO
...
Clone the git repositories for lapo and gitian and then checkout the lapo version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/lapo-crypto/lapo.git
cd lapo
...
Follow the instructions in [doc/release-process.md](release-process.md) in the lapo repository
...
To build LAPO (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the lapo repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lapo/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/lapo-crypto/lapo
...
For example:
```bash
URL=https://github.com/crowning-/lapo.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit lapo=${COMMIT} --url lapo=${URL} ../lapo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lapo=${COMMIT} --url lapo=${URL} ../lapo/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lapo=${COMMIT} --url lapo=${URL} ../lapo/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lapo-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lapo-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/lapo-build.assert
```
...
[lapo/gitian.sigs](https://github.com/lapo-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
