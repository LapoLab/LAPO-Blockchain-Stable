...
//step 2.) Top LPCTX_SIGNATURES_TOTAL masternodes, open connect to top 1 masternode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 masternode, waits for LPCTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'

void ProcessMessageLPCtx(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (fLiteMode) return; //disable all obfuscation/masternode related functionality
    if (!IsSporkActive(SPORK_2_LPCTX)) return;
    if (!masternodeSync.IsBlockchainSynced()) return;

    if (strCommand == "ix") {
        //LogPrintf("ProcessMessageLPCtx::ix\n");
        ...
        BOOST_FOREACH (const CTxOut o, tx.vout) {
            ...
            if (!o.scriptPubKey.IsNormalPaymentScript() && !o.scriptPubKey.IsUnspendable()) {
                LogPrintf("ProcessMessageLPCtx::ix - Invalid Script %s\n", tx.ToString().c_str());
                return;
            }
        }
        ...
        if (fAccepted) {
            ...
            LogPrintf("ProcessMessageLPCtx::ix - Transaction Lock Request: %s %s : accepted %s\n",
                pfrom->addr.ToString().c_str(), pfrom->cleanSubVer.c_str(),
                tx.GetHash().ToString().c_str());

            if (GetTransactionLockSignatures(tx.GetHash()) == LPCTX_SIGNATURES_REQUIRED) {
            ...
            LogPrintf("ProcessMessageLPCtx::ix - Transaction Lock Request: %s %s : rejected %s\n",
                pfrom->addr.ToString().c_str(), pfrom->cleanSubVer.c_str(),
                tx.GetHash().ToString().c_str());
            ...
            if (i != mapTxLocks.end()) {
                ...
                if ((*i).second.CountSignatures() >= LPCTX_SIGNATURES_REQUIRED) {
                    if (!CheckForConflictingLocks(tx)) {
                        LogPrintf("ProcessMessageLPCtx::ix - Found Existing Complete IX Lock\n");
    ...
    } else if (strCommand == "txlvote") // LPCtx Lock Consensus Votes
    {
        CConsensusVote ctx;
        vRecv >> ctx;
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                ...
                if (mapUnknownVotes[ctx.vinMasternode.prevout.hash] > GetTime() &&
                    ...
                    LogPrintf("ProcessMessageLPCtx::ix - masternode is spamming transaction votes: %s %s\n",
        ...
        if (mapTxLockReq.count(ctx.txHash) && GetTransactionLockSignatures(ctx.txHash) == LPCTX_SIGNATURES_REQUIRED) {
...
bool IsIXTXValid(const CTransaction& txCollateral)
{
    ...
    if (nValueOut > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
        LogPrint("lpctx", "IsIXTXValid - Transaction value too high - %s\n", txCollateral.ToString().c_str());
        return false;
    }

    if (missingTx) {
        LogPrint("lpctx", "IsIXTXValid - Unknown inputs in IX transaction - %s\n", txCollateral.ToString().c_str());
        /*
    ...
    if (nValueIn - nValueOut < Params().LPCtxMinFee()) {
        LogPrint("lpctx", "IsIXTXValid - did not include enough fees in transaction %d\n%s\n", nValueOut - nValueIn, txCollateral.ToString().c_str());
        return false;
    }
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    */
    ...
    if (!mapTxLocks.count(tx.GetHash())) {
        ...
        LogPrint("lpctx", "CreateNewLock - Transaction Lock Exists %s !\n", tx.GetHash().ToString().c_str());
    }
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetMasternodeRank(activeMasternode.vin, nBlockHeight, MIN_LPCTX_PROTO_VERSION);

    if (n == -1) {
        LogPrint("lpctx", "LPCtx::DoConsensusVote - Unknown Masternode\n");
        return;
    }

    if (n > LPCTX_SIGNATURES_TOTAL) {
        LogPrint("lpctx", "LPCtx::DoConsensusVote - Masternode not in the top %d (%d)\n", LPCTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    LogPrint("lpctx", "LPCtx::DoConsensusVote - In the top %d (%d)\n", LPCTX_SIGNATURES_TOTAL, n);
    ...
    if (!ctx.Sign()) {
        LogPrintf("LPCtx::DoConsensusVote - Failed to sign consensus vote\n");
        return;
    }
    if (!ctx.SignatureValid()) {
        LogPrintf("LPCtx::DoConsensusVote - Signature invalid\n");
        return;
    }
...
{
    int n = mnodeman.GetMasternodeRank(ctx.vinMasternode, ctx.nBlockHeight, MIN_LPCTX_PROTO_VERSION);

    CMasternode* pmn = mnodeman.Find(ctx.vinMasternode);
    if (pmn != NULL)
        LogPrint("lpctx", "LPCtx::ProcessConsensusVote - Masternode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("lpctx", "LPCtx::ProcessConsensusVote - Unknown Masternode\n");
        mnodeman.AskForMN(pnode, ctx.vinMasternode);
        return false;
    }

    if (n > LPCTX_SIGNATURES_TOTAL) {
        LogPrint("lpctx", "LPCtx::ProcessConsensusVote - Masternode not in the top %d (%d) - %s\n", LPCTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("LPCtx::ProcessConsensusVote - Signature invalid\n");
    ...
    if (!mapTxLocks.count(ctx.txHash)) {
        LogPrintf("LPCtx::ProcessConsensusVote - New Transaction Lock %s !\n", ctx.txHash.ToString().c_str());
        ...
        LogPrint("lpctx", "LPCtx::ProcessConsensusVote - Transaction Lock Exists %s !\n", ctx.txHash.ToString().c_str());
    ...
    if (i != mapTxLocks.end()) {
        ...
        LogPrint("lpctx", "LPCtx::ProcessConsensusVote - Transaction Lock Votes %d - %s !\n", (*i).second.CountSignatures(), ctx.GetHash().ToString().c_str());

        if ((*i).second.CountSignatures() >= LPCTX_SIGNATURES_REQUIRED) {
            LogPrint("lpctx", "LPCtx::ProcessConsensusVote - Transaction Lock Is Complete %s !\n", (*i).second.GetHash().ToString().c_str());
    ...
    BOOST_FOREACH (const CTxIn& in, tx.vin) {
        if (mapLockedInputs.count(in.prevout)) {
            if (mapLockedInputs[in.prevout] != tx.GetHash()) {
                LogPrintf("LPCtx::CheckForConflictingLocks - found two complete conflicting locks - removing both. %s %s", tx.GetHash().ToString().c_str(), mapLockedInputs[in.prevout].ToString().c_str());
...
int GetTransactionLockSignatures(uint256 txHash)
{
    ...
    if (!IsSporkActive(SPORK_2_LPCTX)) return -1;
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pmn == NULL) {
        LogPrintf("LPCtx::CConsensusVote::SignatureValid() - Unknown Masternode\n");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pmn->pubKeyMasternode, vchMasterNodeSignature, strMessage, errorMessage)) {
        LogPrintf("LPCtx::CConsensusVote::SignatureValid() - Verify message failed\n");
        return false;
    }
...
bool CTransactionLock::SignaturesValid()
{
    BOOST_FOREACH (CConsensusVote vote, vecConsensusVotes) {
        int n = mnodeman.GetMasternodeRank(vote.vinMasternode, vote.nBlockHeight, MIN_LPCTX_PROTO_VERSION);
