...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "lapo.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "lapod.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the LAX and zLAX money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 31714, 31715));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in LAX/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in LAX/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via LPCtx) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via LPCtx) in <address>"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, lapo, (obfuscation, lpctx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in LAX/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all LAPO specific functionality (Masternodes, Zerocoin, LPCtx, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:31714"));
    ...
    strUsage += HelpMessageOpt("-backupzlax=<n>", strprintf(_("Enable automatic wallet backups triggered after each zLax minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizelapoamount=<n>     " + strprintf(_("Keep N LAX anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageGroup(_("LPCtx options:"));
    strUsage += HelpMessageOpt("-enablelpctx=<n>", strprintf(_("Enable LPCtx, show confirmations for locked transactions (bool, default: %s)"), "true"));
    strUsage += HelpMessageOpt("-lpctxdepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), nLPCtxDepth));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 31718, 31719));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The LAPO Core Developers"), "2018")) + "\n" +
           "\n" + 
...
/** Sanity checks
 *  Ensure that LAPO is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!GetBoolArg("-enablelpctx", fEnableLPCtx)) {
        if (SoftSetArg("-lpctxdepth", "0"))
            LogPrintf("AppInit2 : parameter interaction: -enablelpctx=false -> setting -nLPCtxDepth=0\n");
    }
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. LAPO Core is shutting down."));
...
#endif
    // Make sure only a single LAPO process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. LAPO Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //LAPO specific: zerocoin and spork DB's
                ...
                // LAPO: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZLAXMinted();
                        RecalculateZLAXSpent();
                    }
                    RecalculateLAXSupply(1);
                }
                ...
                // LAPO: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of LAPO Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart LAPO Core to complete") << "\n";
        ...
        bool fEnableZLaxBackups = GetBoolArg("-backupzlax", true);
        pwalletMain->setZLaxAutoBackups(fEnableZLaxBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeLapoAmount = GetArg("-anonymizelapoamount", 0);
//    if (nAnonymizeLapoAmount > 999999) nAnonymizeLapoAmount = 999999;
//    if (nAnonymizeLapoAmount < 2) nAnonymizeLapoAmount = 2;

    fEnableLPCtx = GetBoolArg("-enablelpctx", fEnableLPCtx);
    nLPCtxDepth = GetArg("-lpctxdepth", nLPCtxDepth);
    nLPCtxDepth = std::min(std::max(nLPCtxDepth, 0), 60);
    ...
    LogPrintf("nLPCtxDepth %d\n", nLPCtxDepth);
    LogPrintf("Anonymize LAPO Amount %d\n", nAnonymizeLapoAmount);
    ...
    /* Denominations
       ...
       For example:
       1LAX+1000 == (.1LAX+100)*10
       10LAX+10000 == (1LAX+1000)*10
    */
