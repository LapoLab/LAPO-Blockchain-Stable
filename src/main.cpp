...
#if defined(NDEBUG)
#error "LAPO cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ulax) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "LAPO Signed Message:\n";
...
int GetInputAgeIX(uint256 nTXHash, CTxIn& vin)
{
    ...
    if (nResult < 6) {
        ...
        if (sigs >= LPCTX_SIGNATURES_REQUIRED) {
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zLax with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zLax spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zLAX spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetSubsidy(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = (10 * 1000000000LL) * COIN;
    } else if (nHeight < 1051201) {     // PoW + PoS
        nSubsidy = 2500 * COIN;
    } else if (nHeight < 4204801) {     // PoS onewards
        nSubsidy = 2500 * COIN;
    } else if (nHeight < 7358401) {
        nSubsidy = 2000 * COIN;
    } else if (nHeight < 10512001) {
        nSubsidy = 1800 * COIN;
    } else if (nHeight < 15768001) {
        nSubsidy = 1600 * COIN;
    } else if (nHeight < 26280001) {
        nSubsidy = 1400 * COIN;
    } else if (nHeight < 36792001) {
        nSubsidy = 1200 * COIN;
    } else if (nHeight < 47304001) {
        nSubsidy = 1000 * COIN;
    } else if (nHeight < 63072001) {
        nSubsidy = 750 * COIN;
    } else if (nHeight < 84096001) {
        nSubsidy = 350 * COIN;
    } else {
        // halving strating 84096001
        int halvings = (nHeight - 84096001) / Params().SubsidyHalvingInterval();  // 262800
        ...
        nSubsidy =  200 * COIN;
        // Subsidy is cut in half every 282,600 blocks
        nSubsidy >>= halvings;
    }
...
int64_t GetBlockValue(int nHeight)
{
    ...
    // after premine remove 10% for budget - allocate in masternode-budget
    if (nHeight > 1)
    {
        return nBlockValue * (100 - Params().SubsidyBudgetPercentage()) / 100;
    }
...
{
    int64_t ret = 0;
    int64_t nSubsidy = GetSubsidy(nHeight); // don't use blockvalue as we want to be exact when less than pow
    ...
    if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 0) {     // PoW + PoS
        ...
        int64_t mNodeCoins = nMasternodeCount * Params().MasternodeCollateral() * COIN;
...
void PopulateInvalidOutPointMap()
{
    if (fListPopulatedAfterLock)
        return;
    if (Params().Zerocoin_Block_FirstFraudulent() < 0)
        return;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from LAPO
...
{
    RenameThread("lapo-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZLAXMinted()
{
...
void RecalculateZLAXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zLAX supply
...
bool RecalculateLAXSupply(int nHeightStart)
{
...
bool UpdateZLAXSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (Params().Zerocoin_Block_RecalculateAccumulators() >= 0 && pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateLAXSupply(Params().Zerocoin_StartHeight());
    }

    //Track zLAX money supply in the block index
    if (!UpdateZLAXSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zLAX supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zLaxSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    if (Params().Zerocoin_Block_FirstFraudulent() >=0 && Params().Zerocoin_Block_RecalculateAccumulators() >= 0 &&
        pindex->nHeight >= Params().Zerocoin_Block_FirstFraudulent() && pindex->nHeight <= Params().Zerocoin_Block_RecalculateAccumulators() + 1)
        AddInvalidSpendsToMap(block);
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert LAX to zLAX
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  log2_work=%.8g  tx=%lu timestamp=%d date=%s progress=%f  cache=%u\n",
        chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), log(chainActive.Tip()->nChainWork.getdouble()) / log(2.0), (unsigned long)chainActive.Tip()->nChainTx,
        chainActive.Tip()->GetBlockTime(), DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
...
/*
    ...
    Remove conflicting blocks for successful LPCtx transaction locks
...
{
    LogPrintf("CheckBlock(): fCheckPOW=%d\n", fCheckPOW);
    ...
    // ----------- LPCtx transaction scanning -----------
    if (IsSporkActive(SPORK_3_LPCTX_BLOCK_FILTERING)) {
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            if (!tx.IsCoinBase()) {
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zLax spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zLax serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zLax mints and %d zLax spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // LAPO: We use certain sporks during IBD, so check to see if they are
    ...
    // getaddr message mitigates the attack.
        ...
        ProcessMessageLPCtx(pfrom, strCommand, vRecv);
...
*/
    ...
    if (IsSporkActive(SPORK_17_NEW_PROTOCOL_TARGETTIME))
            return MIN_PEER_PROTO_VERSION_TARGETTIME;
