...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LAX);
    ...
    if (!settings.contains("nAnonymizeLapoAmount"))
        settings.setValue("nAnonymizeLapoAmount", 1000);

    nAnonymizeLapoAmount = settings.value("nAnonymizeLapoAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeLapoAmount"))
        SoftSetArg("-anonymizelapoamount", settings.value("nAnonymizeLapoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in lapo.cpp during shotdown to also remove the window positions
...
{        
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLapoAmount:
            return QVariant(nAnonymizeLapoAmount);
        case Listen:
            return settings.value("fListen");
        case MiningThreads:
            printf("data MiningThreads");
            return settings.value("nMiningThreads");
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLapoAmount:
            nAnonymizeLapoAmount = value.toInt();
            settings.setValue("nAnonymizeLapoAmount", nAnonymizeLapoAmount);
            emit anonymizeLapoAmountChanged(nAnonymizeLapoAmount);
            break;
        case CoinControlFeatures:
        ...
        case Listen:
            ...
            break;
        case MiningThreads:
            printf("setData MiningThreads\n");
            if (settings.value("nMiningThreads") != value.toInt()) {
                settings.setValue("nMiningThreads", value.toInt());
                setGenerate(value.toInt());
            }
            break;            
        default:
            break;
...
void OptionsModel::setGenerate(int nMiningThreads)
{
    if (pwalletMain) {
        if (nMiningThreads) {
            printf("Mining using %d thread(s)...\n", nMiningThreads);
            GenerateBitcoins(true, pwalletMain, nMiningThreads);
        }
        else {
            printf("Disabling mini\n", nMiningThreads);
