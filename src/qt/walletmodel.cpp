...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered lapo address / amount:
...

    {
        ...
        if (recipients[0].useLPCtx && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("LPCtx doesn't support sending values that high yet. Transactions are currently limited to %1 LAX.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }

        bool fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, strFailReason, coinControl, recipients[0].inputType, recipients[0].useLPCtx);
        transaction.setTransactionFee(nFeeRequired);

        if (recipients[0].useLPCtx && newTx->GetValueOut() > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("LPCtx doesn't support sending values that high yet. Transactions are currently limited to %1 LAX.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal lapo:URI (lapo:XyZ...?message=example)
            {
        ...
        if (!wallet->CommitTransaction(*newTx, *keyChange, (recipients[0].useLPCtx) ? "ix" : "tx"))
...
static void NotifyzLAXReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzLAXReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzLAXReset.connect(boost::bind(NotifyzLAXReset, this));
}
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzLAXReset.disconnect(boost::bind(NotifyzLAXReset, this));
}
