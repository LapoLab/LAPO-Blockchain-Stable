                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    if (!settings.contains("bUseLPCtx"))
        settings.setValue("bUseLPCtx", false);

    bool useLPCtx = settings.value("bUseLPCtx").toBool();
    if (fLiteMode) {
        ui->checkLPCtx->setVisible(false);
        CoinControlDialog::coinControl->useObfuScation = false;
        CoinControlDialog::coinControl->useLPCtx = false;
    } else {
        ui->checkLPCtx->setChecked(useLPCtx);
        CoinControlDialog::coinControl->useLPCtx = useLPCtx;
    }

    connect(ui->checkLPCtx, SIGNAL(stateChanged(int)), this, SLOT(updateLPCtx()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (ui->checkLPCtx->isChecked()) {
        recipients[0].useLPCtx = true;
        strFunds += " ";
        strFunds += tr("using LPCtx");
    } else {
        recipients[0].useLPCtx = false;
    }
    ...
    if (encStatus == model->Locked || encStatus == model->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(model->requestUnlock(AskPassphraseDialog::Context::Send_LAX, true));
        if (!ctx.isValid()) {
...
void SendCoinsDialog::updateLPCtx()
{
    QSettings settings;
    settings.setValue("bUseLPCtx", ui->checkLPCtx->isChecked());
    CoinControlDialog::coinControl->useLPCtx = ui->checkLPCtx->isChecked();
    coinControlUpdateLabels();
    updateFeeMinimizedLabel();
}
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->checkLPCtx->isChecked()) {
        nMinTxFee = max(Params().LPCtxMinFee(), nMinTxFee);
    }
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid LAPO address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
