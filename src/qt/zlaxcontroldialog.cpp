...
#include "ui_zlaxcontroldialog.h"
...
std::list<std::string> ZLaxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZLaxControlDialog::listMints;

ZLaxControlDialog::ZLaxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZLaxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZLaxControlDialog::~ZLaxControlDialog()
{
    delete ui;
}

void ZLaxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLaxControlDialog::updateList()
{
...
void ZLaxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLaxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZLax_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZLaxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZLaxControlDialog::GetSelectedMints()
{
...
void ZLaxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
