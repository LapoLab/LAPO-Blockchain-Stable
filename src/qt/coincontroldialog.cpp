...
using namespace std;
CAmount CoinControlDialog::payFee;
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    if (nQuantity > 0) {
        ...
        // IX Fee
        if (coinControl->useLPCtx) nPayFee = max(nPayFee, Params().LPCtxMinFee());
        // Allow free?
    ...
    int nDisplayUnit = BitcoinUnits::LAX;
    ...
    QString toolTip4 = tr("Can vary +/- %1 ulax per input.").arg(dFeeVary);
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show LAPO address. In tree mode, address is not shown again for direct wallet address outputs
