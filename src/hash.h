...
GLOBAL sph_blake256_context z_blake256;
...
#define ZBLAKE256 (memcpy(&ctx_blake256, &z_blake256, sizeof(z_blake256)))
...
/* ----------- Lyra2z Hash ------------------------------------------------ */
template<typename T1>
inline uint256 HashLyra2z(const T1 pbegin, const T1 pend)
{
    ...
    sph_blake256 (&ctx_blake256, (pbegin == pend ? pblank : static_cast<const void*>(&pbegin[0])), (pend - pbegin) * sizeof(pbegin[0]));
    sph_blake256_close(&ctx_blake256, static_cast<void*>(&hash[0]));

    LYRA2(static_cast<void*>(&hash[1]), 32, static_cast<void*>(&hash[0]), 32, static_cast<void*>(&hash[0]), 32, 8, 8, 8);
