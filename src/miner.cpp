...
{
//    LogPrintf("CreateNewBlock(): fProofOfStake=%d\n", fProofOfStake);
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zlaxs that have been in mempool long
                    //and higher priority to zlaxs that are large in value
                    ...
                    // zLAX spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zLax spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zLax serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
//        LogPrintf("CreateNewBlock(): vin[0].scriptSig size %d\n", pblock->vtx[0].vin[0].scriptSig.size());
    ...
    LogPrintf("%s: Releasing pblocktemplate\n", __func__);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("LAPOMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("LAPOMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("LAPOMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lapo-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (chainActive.Tip()->nTime < Params().GenesisBlock().nTime || vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || 
        ...
        LogPrintf("Running LAPOMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
