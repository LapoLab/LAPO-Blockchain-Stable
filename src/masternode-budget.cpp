...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf)
{
    ...
    */
    ...
    //if we're syncing we won't have LPCtx information, so accept 1 confirmation
...
void CBudgetManager::SubmitFinalBudget()
{
    ...
    if (!mapCollateralTxids.count(tempBudget.GetHash())) {
        ...
        // Send the tx to the network. Do NOT use LPCtx, locking might need too much time to propagate, especially for testnet
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    // no budget during PoW
    if (nHeight == 0)
        return 0;

    // get block value and calculate from that
    int64_t nBudgetSubsidy = 0;        
    nBudgetSubsidy = GetBlockValue(nHeight) * Params().SubsidyBudgetPercentage() / 100;
